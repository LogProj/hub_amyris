...
import { HUB_SCREEN_KEYS } from "@/lib/screens"
...
export type Authorization = {
  ...
  isAdmin: boolean
  hasAccess: boolean
  // Telas locais liberadas para este usuário. Admin sempre recebe TODAS (HUB_SCREEN_KEYS),
...
async function resolveAuthorization(user: GlobalAuthUser): Promise<Authorization | null> {
  ...
  try {
    ...
    return {
      ...
      visibleScreens: registro.isAdmin ? HUB_SCREEN_KEYS : (registro.visibleScreens ?? []),
    }
  } catch {
    ...
    if (ehBootstrap) {
      return {
        ...
        isAdmin: true,
        hasAccess: true,
        visibleScreens: HUB_SCREEN_KEYS,
      }
    }
    return null
  }

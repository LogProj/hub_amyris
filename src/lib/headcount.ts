 ...
 *  - hora (ex.: "08:04")  → presença (roxo)
 *  - "FALTA"              → falta (vermelho)
 *  - vazio / "NaN" / nulo → folga / sem registro (cinza)
 *
 * Categorias profissionais EXCLUÍDAS da análise (perfis administrativos, fora do
 * escopo operacional deste indicador): "ANALISTA DE PROJETOS PL" e
 * "SUPERVISOR DE LOGISTICA" (grafia exata confirmada na view).
 *
 * Diferente da Ultragaz, a view da Amyris não tem coluna `turno` nem
 * `importado_em`/`observacoes` — por isso este indicador não tem quebra por
 * turno, e a "aderência"/"absenteísmo" usam como denominador os colaboradores
 * ESCALADOS no dia (presença + falta), em vez de um quadro contratado fixo.
 */
...
export type Headcount = {
  ...
  quadro: number // colaboradores distintos no mês (categorias elegíveis)
 ...
 * distorce KPIs/gráficos/grade — por isso toda consulta à view exclui "hoje".
...
export async function getMesesDisponiveis(): Promise<string[]> {
  const { rows } = await inhausPool.query<{ mes: string }>(
    `select distinct to_char(data, 'YYYY-MM') as mes
       from public.vw_ponto_amyris
      where coalesce(categoria_profissional, '') not in ($1, $2)
        and data <= $3
      order by mes desc`,
    [...CATEGORIAS_EXCLUIDAS, getMaxAllowedDate()],
  )
  ...
  const { rows } = await inhausPool.query<Linha>(
    `select colaborador,
      ...
      where to_char(data, 'YYYY-MM') = $1
        and coalesce(categoria_profissional, '') not in ($2, $3)
        and data <= $4
      order by colaborador, data`,
    [mes, ...CATEGORIAS_EXCLUIDAS, getMaxAllowedDate()],
  )
 ...
 * contratado fixo definido para a Amyris.
 */

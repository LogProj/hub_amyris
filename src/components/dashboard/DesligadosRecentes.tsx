...
import type { Desligado } from "@/lib/turnover"
...
export function DesligadosRecentes({ dados }: { dados: Desligado[] }) {
  if (dados.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-amyris/20 bg-amyris-mist/30 px-6 py-10 text-center">
        <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-amyris-mist text-amyris">
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-amyris/10">
      <div className="max-h-[420px] overflow-auto">
        <table className="w-full text-left text-sm">
          <thead className="sticky top-0 bg-amyris-mist/70 text-xs uppercase tracking-wide text-muted-foreground backdrop-blur">
            <tr>
          ...
          <tbody className="divide-y divide-amyris/10">
            {dados.map((d) => (
              <tr key={`${d.nome}-${d.dtDemissao}`} className="transition-colors hover:bg-amyris-mist/30">

...
const ToggleDonut = nextDynamic(() =>
  import("@/components/dashboard/OcorrenciasCharts").then((m) => m.ToggleDonut),
)
const ToggleBarras = nextDynamic(() =>
  import("@/components/dashboard/OcorrenciasCharts").then((m) => m.ToggleBarras),
)
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="reveal flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Indicadores de segurança — {data.clienteLabel}.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Kpi icone={FileText} rotulo="Total de Ocorrências" valor={data.kpis.total} detalhe="Todas as ocorrências" dica="Soma de todas as ocorrências já registradas para o cliente, somando todos os meses." />
        <Kpi
          icone={AlertTriangle}
          ...
          variacao={variacao}
          dica="Quantas ocorrências aconteceram no mês selecionado. A setinha compara com o mês anterior: para baixo (verde) é bom; para cima (vermelho) é alerta."
        />
        <Kpi icone={FileText} rotulo="Com CAT Aberta" valor={data.kpis.comCat} detalhe="Comunicação de Acidente" dica="Ocorrências do mês que geraram CAT (Comunicação de Acidente de Trabalho) aberta." />
        <Kpi icone={Users} rotulo="Colaboradores" valor={data.kpis.colaboradores} detalhe="Colaboradores únicos" dica="Quantas pessoas diferentes apareceram nas ocorrências do mês." />
      </div>

      {/* Calendário + Recentes */}
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="reveal glass flex h-[400px] flex-col rounded-2xl p-5">
          <TituloSecao icone={CalendarDays} texto="Calendário de Ocorrências" dica="Mostra em quais dias do mês houve ocorrências. Quanto mais forte a cor, mais ocorrências naquele dia." />
          <Calendario dias={data.calendario} />
        </section>

        <section className="reveal delay-1 glass flex h-[400px] flex-col rounded-2xl p-5">
          <TituloSecao icone={Activity} texto="Ocorrências Recentes" dica="Últimas ocorrências registradas, da mais nova para a mais antiga." />
          <ul className="mt-3 flex-1 divide-y divide-amyris/5 overflow-y-auto pr-1">
            {data.recentes.length === 0 ? (
              <li className="py-10 text-center text-sm text-muted-foreground">Nenhuma ocorrência no mês.</li>
            ) : (
              data.recentes.map((o) => (
      ...
      <section className="reveal glass rounded-2xl p-5">
        <TituloSecao icone={Activity} texto="Ocorrências por Mês" dica="Evolução do número de ocorrências mês a mês ao longo do ano." />
        <div className="mt-4">
          <MesBarChart data={data.porMes} />
        </div>
      </section>

      {/* Três colunas: Turno/PSIF · GPS/Cliente · Top Colaboradores */}
      <div className="grid gap-4 md:grid-cols-3">
        <ToggleDonut
          ladoA={{ label: "Turno", icon: "Clock", data: data.porTurno }}
          ladoB={{ label: "PSIF", icon: "Activity", data: data.porPsif }}
          dica="Distribuição das ocorrências do mês. Use o botão para alternar entre Turno e PSIF."
        />
        <ToggleBarras
          ladoA={{ label: "Classif. GPS", icon: "Tag", data: data.porClassGps }}
          ladoB={{ label: "Classif. Cliente", icon: "Target", data: data.porClassCliente }}
          dica="Ocorrências por classificação. Alterna entre a classificação GPS e a classificação do cliente."
        />
        <TopColaboradores itens={data.topColaboradores} />
      </div>

      {/* Duas colunas: Parte do Corpo/Negócio · Por Tipo */}
      <div className="grid gap-4 lg:grid-cols-2">
        <ToggleBarras
          ladoA={{ label: "Parte do Corpo", icon: "Users", data: data.porParteCorpo }}
          ladoB={{ label: "Negócio", icon: "Briefcase", data: data.porNegocio }}
          dica="Alterna entre a parte do corpo atingida (casos com lesão) e a área da operação."
        />
        <Barras titulo="Por Tipo de Ocorrência" icone={AlertTriangle} itens={data.porTipo} dica="Tipos de ocorrência mais frequentes no mês, do maior para o menor." />
      </div>

      {/* Tabela */}
      <section className="reveal glass rounded-2xl p-5">
        <TituloSecao icone={FileText} texto="Ocorrências do Período" dica="Todas as ocorrências do mês, uma por linha." />
          ...
          <table className="w-full min-w-[820px] text-sm">
            <thead>
              <tr className="border-b border-amyris/10 text-left text-xs uppercase tracking-wide text-muted-foreground">
                ...
                <th className="py-2 pr-3 font-medium">Class. GPS</th>
            ...
            <tbody className="divide-y divide-amyris/5">
              {data.todas.length === 0 ? (
                <tr>
                  <td colSpan={9} className="py-8 text-center text-sm text-muted-foreground">Nenhuma ocorrência no mês.</td>
                </tr>
              ) : (
                data.todas.map((o) => (
                  <tr key={o.id} className="text-foreground/90">
                    ...
                    <td className="py-2.5 pr-3">
                      {o.cat ? (
                        <span className="rounded-full bg-orange-100 px-2 py-0.5 text-[11px] font-semibold text-orange-700">Sim</span>
                      ) : (
...
function TopColaboradores({ itens }: { itens: { nome: string; count: number }[] }) {
  const max = Math.max(1, ...itens.map((i) => i.count))
  return (
    <section className="reveal glass flex flex-col rounded-2xl p-5">
      <TituloSecao icone={Users} texto="Top Colaboradores" dica="Pessoas com mais ocorrências registradas no mês." />
      {itens.length === 0 ? (
        ...
        <ul className="mt-4 space-y-3">
          {itens.map((c, i) => (
            <li key={i} className="flex items-center gap-2.5">
              <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${i < 3 ? "bg-amyris-grad text-white" : "bg-amyris-mist text-amyris"}`}>
                {i + 1}
              </span>
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between text-sm">
                  <span className="truncate pr-2 font-medium text-foreground/90">{c.nome}</span>
                  <span className="font-semibold text-amyris">{c.count}</span>
                </div>
                <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-amyris-mist">
                  <div className="h-full rounded-full bg-amyris-grad" style={{ width: `${(c.count / max) * 100}%` }} />
                </div>
              </div>
            </li>
          ))}
...
function Barras({
  ...
  const data = itens.slice(0, 5)
  const max = Math.max(1, ...data.map((i) => i.valor))
  return (
...
function Pill({ texto }: { texto: string }) {
  return <span className="rounded-full bg-amyris-mist px-2.5 py-0.5 text-[11px] font-medium text-amyris">{texto}</span>
}
